# Personal Claude Preferences

## Code Comments

Prefer self-documenting code over comments. When a comment is needed, it should explain *why*, not *what*.

Do not add comments about:
- Local testing steps or how to run something
- Session context, iteration history, or what changed in this pass
- Anything that documents the current task rather than the code itself

Comments should be clean, concise, and durable — written as if the task that prompted them never existed.
